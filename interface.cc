...
{
  *game_data = {TScreen::IMAGE, 0, 1, 1, 0, 0, 0, 0.0f};
}
...
void DrawHeader(TGame game_data, esat::SpriteHandle sprite_lives, int lives = 0)
{
  ...
  if (lives > 0)
  {
    ...
    *(pts + 0) = sprite_x;
    *(pts + 1) = sprite_y;
    *(pts + 2) = sprite_x + esat::SpriteWidth(sprite_lives);
    *(pts + 3) = sprite_y;
    *(pts + 4) = sprite_x + esat::SpriteWidth(sprite_lives);
    *(pts + 5) = sprite_y + esat::SpriteHeight(sprite_lives);
    *(pts + 6) = sprite_x;
    *(pts + 7) = sprite_y + esat::SpriteHeight(sprite_lives);
